function GreenCFn($scope, $http, $sce) {
    ...
    $scope.searchfn = function () {
        ...
        $http.get("/greencstations/" + FromStation).success(function (response) {
            ...
            $http.get("/greencstations/" + ToStation).success(function (response) {
                console.log(response);
                ...
                $http.get("http://realtime.mbta.com/developer/api/v2/schedulebyroute?api_key=TbUOod1u00WYU5oe6x7h1g&route=green-c&format=json")
                .success(function (response) {
                    ...
                    var greencstops = response.direction[direction_id].trip[2].stop;
                    for (var i = 0; i < greencstops.length; i++) {
                        var stop_name = greencstops[i].stop_name;

                        if (stop_name.indexOf(FromStation) >= 0) {
                            var FromDate = new Date(greencstops[i].sch_arr_dt * 1000);
                        ...
                        if (stop_name.indexOf(ToStation) >= 0) {
                            var ToDate = new Date(greencstops[i].sch_arr_dt * 1000);
...
angular.module('GreenCApp', []).controller("GreenCCtrl", GreenCFn);