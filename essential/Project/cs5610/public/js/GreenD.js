function GreenDFn($scope, $http, $sce) {
    ...
    $scope.searchfn = function () {
        ...
        $http.get("/greendstations/" + FromStation).success(function (response) {
            ...
            $http.get("/greendstations/" + ToStation).success(function (response) {
                console.log(response);
                ...
                $http.get("http://realtime.mbta.com/developer/api/v2/schedulebyroute?api_key=TbUOod1u00WYU5oe6x7h1g&route=green-d&format=json")
                .success(function (response) {
                    ...
                    var greendstops = response.direction[direction_id].trip[2].stop;
                    for (var i = 0; i < greendstops.length; i++) {
                        var stop_name = greendstops[i].stop_name;

                        if (stop_name.indexOf(FromStation) >= 0) {
                            var FromDate = new Date(greendstops[i].sch_arr_dt * 1000);
                        ...
                        if (stop_name.indexOf(ToStation) >= 0) {
                            var ToDate = new Date(greendstops[i].sch_arr_dt * 1000);
...
angular.module('GreenDApp', []).controller("GreenDCtrl", GreenDFn);