function GreenEFn($scope, $http, $sce) {
    ...
    $scope.searchfn = function () {
        ...
        $http.get("/greenestations/" + FromStation).success(function (response) {
            ...
            $http.get("/greenestations/" + ToStation).success(function (response) {
                console.log(response);
                ...
                $http.get("http://realtime.mbta.com/developer/api/v2/schedulebyroute?api_key=TbUOod1u00WYU5oe6x7h1g&route=green-e&format=json")
                .success(function (response) {
                    ...
                    var greenestops = response.direction[direction_id].trip[2].stop;
                    for (var i = 0; i < greenestops.length; i++) {
                        var stop_name = greenestops[i].stop_name;

                        if (stop_name.indexOf(FromStation) >= 0) {
                            var FromDate = new Date(greenestops[i].sch_arr_dt * 1000);
                        ...
                        if (stop_name.indexOf(ToStation) >= 0) {
                            var ToDate = new Date(greenestops[i].sch_arr_dt * 1000);
...
angular.module('GreenEApp', []).controller("GreenECtrl", GreenEFn);