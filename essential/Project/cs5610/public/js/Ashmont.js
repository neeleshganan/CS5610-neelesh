function AshmontFn($scope, $http, $sce) {
    ...
    $scope.searchfn = function () {
        ...
        $http.get("/ashmontstations/" + FromStation).success(function (response) {
            ...
            $http.get("/ashmontstations/" + ToStation).success(function (response) {
                console.log(response);
                ...
                .success(function (response) {
                    ...
                    var ashmontstops = response.direction[direction_id].trip[trip_id].stop;
                    for (var i = 0; i < ashmontstops.length; i++) {
                        var stop_name = ashmontstops[i].stop_name;

                        if (stop_name.indexOf(FromStation) >= 0) {
                            var FromDate = new Date(ashmontstops[i].sch_arr_dt * 1000);
                        ...
                        if (stop_name.indexOf(ToStation) >= 0) {
                            var ToDate = new Date(ashmontstops[i].sch_arr_dt * 1000);
...
angular.module('AshmontApp', []).controller("AshmontCtrl", AshmontFn);